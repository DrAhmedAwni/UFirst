...
Status: implemented UFirst experience / final camera art approval pending
Scope: UFirst only  
...
Implementation checkpoint: the persistent UFirst camera journey, semantic chapter layer, adaptive quality tiers, reduced-motion behavior, editable experience copy, physical service/work surfaces, non-WebGL fallback, original separated camera GLBs, Catmull–Rom camera path, and a shared compressed PBR body surface set are now implemented. The remaining production work is final optical/internal surface refinement, profile-guided LOD tuning, and agency approval of the generated asset against `3D-ASSET-REQUIREMENTS.md`.
...
- Production-format camera assets: `public/models/ufirst-camera-production.glb`, `public/models/ufirst-camera-mobile.glb`, and `public/models/ufirst-camera-distant.glb`, generated by `scripts/generate-ufirst-camera.mjs`; the loader selects the quality tier, verifies the named journey groups, and uses the procedural model only if loading fails.
