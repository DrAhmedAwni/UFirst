...
export default function HomeExperience({ content }: { content: SiteContent }) {
  ...
  return (
    <main className="site-shell" data-experience="ufirst-scroll-3d-v2" style={{ '--pattern-image': `url('${content.backgrounds.pattern.src}')` } as CSSProperties}>
      <SystemScene assets={systemAssets} paused={motionPaused} />
      ...
      <section className="hero" id="top" onPointerMove={handleHeroPointerMove} onPointerLeave={() => setPointer({ x: 50, y: 50 })} style={{ '--pointer-x': `${pointer.x}%`, '--pointer-y': `${pointer.y}%` } as CSSProperties}>
        ...
        <div className="section-shell hero-content">
          ...
          <div className="hero-copy-wrap">
            ...
            <SceneStage chapter="hero" image={content.system.dashboard.src} label="Dimensional UFirst mark with orbiting creative tools; scroll to rotate and assemble." />
          </div>
          <div className="motion-toolbar"><span>IDEA → CREATION → MOMENTUM</span><button type="button" aria-pressed={motionPaused} onClick={() => setMotionPaused((paused) => !paused)}>{motionPaused ? 'Resume 3D motion' : 'Pause 3D motion'}</button></div>
      ...
      <section className="section section-cream" id="process"><div className="section-shell"><div className="section-kicker section-kicker-light" data-reveal="up"><span>06</span> How we move</div><div className="process-heading" data-reveal="up"><h2>Less theatre.<br /><em>More traction.</em></h2><p>Simple enough to keep momentum, rigorous enough to protect the idea, and flexible enough for the real world.</p></div><SceneStage chapter="process" image={content.system.dashboard.src} label="Five connected creative objects form a moving production sequence as you scroll." /><div className="process-list" data-reveal="stagger">{content.process.map((step) => <div className="process-step" key={step.number}><span className="process-number">{step.number}</span><h3>{step.title}</h3><p>{step.text}</p><span className="process-line" /></div>)}</div></div></section>
      ...
      <section className="section section-dark finale-section" aria-label="UFirst creative system finale"><div className="section-shell finale-grid"><div><p className="section-kicker">One team. Every dimension.</p><h2>All the pieces.<br /><em>One bold move.</em></h2><a className="button button-primary" href="#contact">Let’s build yours <span>↗</span></a></div><SceneStage chapter="finale" image={content.system.browser.src} label="UFirst mark and creative tools assemble into the final connected composition." /></div></section>
