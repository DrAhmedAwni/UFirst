      ...
      <div className="admin-layout">
        ...
        <section className="admin-main">
          ...
          {activeTab === 'content' ? <div className="admin-content-grid">
            <section className="admin-storage-notice admin-card-wide"><span className="admin-status-dot" /><p>Text and content publishing are available. Image replacement is paused until R2 storage is enabled; the current UFirst images and 3D assets remain live.</p></section>
            <section className="admin-card admin-card-wide"><div className="admin-card-heading"><div><p className="admin-kicker">01 / Hero</p><h3>The first impression</h3></div><ImageUpload disabled={!mediaEnabled} label="Replace hero background" src={content.hero.background.src} onChange={(event) => handleImageChange(event, 'hero background', (src) => setContent((current) => ({ ...current, hero: { ...current.hero, background: { ...current.hero.background, src } } })))} /></div><div className="admin-form-grid"><Field label="Eyebrow" value={content.hero.eyebrow} onChange={(value) => updateHero('eyebrow', value)} /><Field label="Headline" value={content.hero.headline} onChange={(value) => updateHero('headline', value)} /><Field label="Emphasis" value={content.hero.emphasis} onChange={(value) => updateHero('emphasis', value)} /><Field label="Primary CTA" value={content.hero.primaryCta} onChange={(value) => updateHero('primaryCta', value)} /><Field label="Secondary CTA" value={content.hero.secondaryCta} onChange={(value) => updateHero('secondaryCta', value)} /><Field wide label="Supporting text" value={content.hero.body} onChange={(value) => updateHero('body', value)} multiline /></div></section>
            <section className="admin-card"><div className="admin-card-heading"><div><p className="admin-kicker">01A / Backgrounds</p><h3>Set the texture</h3></div><ImageUpload disabled={!mediaEnabled} label="Replace pattern" src={content.backgrounds.pattern.src} onChange={(event) => handleImageChange(event, 'background pattern', (src) => setContent((current) => ({ ...current, backgrounds: { ...current.backgrounds, pattern: { ...current.backgrounds.pattern, src } } })))} /></div><p className="admin-card-hint">This pattern is reused across the hero and dark content sections.</p></section>
            <section className="admin-card"><div className="admin-card-heading"><div><p className="admin-kicker">02 / About</p><h3>Make the work matter.</h3></div><ImageUpload disabled={!mediaEnabled} label="Replace about image" src={content.about.image.src} onChange={(event) => handleImageChange(event, 'about image', (src) => setContent((current) => ({ ...current, about: { ...current.about, image: { ...current.about.image, src } } })))} /></div><div className="admin-form-stack"><Field label="Headline" value={content.about.headline} onChange={(value) => updateAbout('headline', value)} /><Field label="Emphasis" value={content.about.emphasis} onChange={(value) => updateAbout('emphasis', value)} /><Field label="Body" value={content.about.body} onChange={(value) => updateAbout('body', value)} multiline /></div></section>
            <section className="admin-card admin-card-wide"><div className="admin-card-heading"><div><p className="admin-kicker">02A / 3D system</p><h3>Replace the scene surfaces</h3></div><span className="admin-card-hint">These images are mapped onto the live scroll-driven 3D modules.</span></div><div className="admin-system-assets">{([['dashboard', 'Dashboard surface'], ['mobile', 'Mobile surface'], ['browser', 'Browser surface']] as const).map(([field, label]) => <div className="admin-system-asset" key={field}><img src={content.system[field].src} alt="" /><ImageUpload disabled={!mediaEnabled} label={label} src={content.system[field].src} onChange={(event) => handleImageChange(event, `3D ${label.toLowerCase()}`, (src) => updateSystemAsset(field, src))} /></div>)}</div></section>
            <section className="admin-card admin-card-wide"><div className="admin-card-heading"><div><p className="admin-kicker">03 / Services</p><h3>Update each capability</h3></div><span className="admin-card-hint">Images are used in the service chapters.</span></div><div className="admin-service-list">{content.services.map((service, index) => <article className="admin-service-row" key={service.number}><div className="admin-service-number">{service.number}</div><div className="admin-service-fields"><Field label="Title" value={service.title} onChange={(value) => updateService(index, { title: value })} /><Field label="Description" value={service.text} onChange={(value) => updateService(index, { text: value })} multiline /><Field label="Tags (comma separated)" value={service.tags.join(', ')} onChange={(value) => updateService(index, { tags: value.split(',').map((tag) => tag.trim()).filter(Boolean) })} /></div><div className="admin-service-media"><img src={service.image.src} alt="" /><ImageUpload disabled={!mediaEnabled} label="Replace image" src={service.image.src} onChange={(event) => handleImageChange(event, `service ${service.number}`, (src) => updateService(index, { image: { ...service.image, src } }))} /></div></article>)}</div></section>
            <section className="admin-card admin-card-wide"><div className="admin-card-heading"><div><p className="admin-kicker">04 / Work imagery</p><h3>Keep selected work current</h3></div><span className="admin-card-hint">Project names and descriptions remain editable in the content model.</span></div><div className="admin-project-list">{content.projects.map((project, index) => <div className="admin-project-row" key={project.title}><img src={project.image.src} alt="" /><div><span className="admin-kicker">{project.category}</span><strong>{project.title}</strong></div><ImageUpload disabled={!mediaEnabled} label="Replace image" src={project.image.src} onChange={(event) => handleImageChange(event, `project ${index + 1}`, (src) => setContent((current) => ({ ...current, projects: current.projects.map((item, itemIndex) => itemIndex === index ? { ...item, image: { ...item.image, src } } : item) })))} /></div>)}</div></section>
...
function ImageUpload({ label, src, onChange, disabled = false }: { label: string; src: string; onChange: (event: ChangeEvent<HTMLInputElement>) => void; disabled?: boolean }) {
  return <label className={disabled ? 'image-upload image-upload-disabled' : 'image-upload'}><span>{label}</span><input type="file" accept="image/*" onChange={onChange} disabled={disabled} /><small>{disabled ? 'Paused until R2 is enabled' : src.startsWith('/api/media/') ? 'Managed media' : 'Current asset'}</small></label>;
}
