      ...
      <div className="admin-layout">
        ...
        <section className="admin-main">
          ...
          {activeTab === 'content' ? <div className="admin-content-grid">
            ...
            <section className="admin-card"><div className="admin-card-heading"><div><p className="admin-kicker">01A / Backgrounds</p><h3>Set the texture</h3></div><ImageUpload label="Replace pattern" src={content.backgrounds.pattern.src} onChange={(event) => handleImageChange(event, 'background pattern', (src) => setContent((current) => ({ ...current, backgrounds: { ...current.backgrounds, pattern: { ...current.backgrounds.pattern, src } } })))} /></div><p className="admin-card-hint">This pattern is reused across the hero and dark content sections.</p></section>
            ...
            <section className="admin-card admin-card-wide"><div className="admin-card-heading"><div><p className="admin-kicker">02A / 3D system</p><h3>Replace the scene surfaces</h3></div><span className="admin-card-hint">These images are mapped onto the live scroll-driven 3D modules.</span></div><div className="admin-system-assets">{([['dashboard', 'Dashboard surface'], ['mobile', 'Mobile surface'], ['browser', 'Browser surface']] as const).map(([field, label]) => <div className="admin-system-asset" key={field}><img src={content.system[field].src} alt="" /><ImageUpload label={label} src={content.system[field].src} onChange={(event) => handleImageChange(event, `3D ${label.toLowerCase()}`, (src) => updateSystemAsset(field, src))} /></div>)}</div></section>
